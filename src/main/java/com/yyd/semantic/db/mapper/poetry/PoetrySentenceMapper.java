...
public interface PoetrySentenceMapper {
	@Select("SELECT id, sentence, poetry_id FROM yyd_resources.tb_sentence WHERE sentence = #{sentence}")
	...
	@Select("SELECT id, sentence, poetry_id FROM yyd_resources.tb_sentence WHERE poetry_id = #{poetryId} ORDER BY id ASC")
	@Results({ @Result(property = "id", column = "id"), @Result(property = "sentence", column = "sentence"),
			@Result(property = "poetryId", column = "poetry_id") })
	public List<PoetrySentence> getByPoetryId(Integer poetryId);

	@Select("SELECT sentence FROM yyd_resources.tb_sentence")
