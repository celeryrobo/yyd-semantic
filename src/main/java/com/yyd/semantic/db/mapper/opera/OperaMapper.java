...
public interface OperaMapper {
	...
	@Select("SELECT id, name, src_url FROM yyd_resources.tb_opera_resource WHERE id = #{id}")
	@Results({
	...
	@Select("SELECT id, name, src_url FROM yyd_resources.tb_opera_resource WHERE name = #{name}")
	@Results({
		@Result(property = "id", column = "id"),
		@Result(property = "name", column = "name"),
		@Result(property = "resourceUrl", column = "src_url")
