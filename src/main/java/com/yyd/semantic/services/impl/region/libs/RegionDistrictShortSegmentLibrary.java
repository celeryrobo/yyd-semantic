...
@Component
public class RegionDistrictShortSegmentLibrary implements SegmentLibrary{
	@Autowired
	private DistrictService districtService;

	@Override
	public List<Value> load() {
		List<String> names = districtService.getAllShortName();
		List<Value> values = new LinkedList<>();
		for (String name : names) {
			values.add(new Value(name, "districtShort", "1"));
		}
		return values;
	}
}
